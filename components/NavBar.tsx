...
const NavBar = () => {
  ...
  return (
    <nav className='flex w-screen h-15 p-4 fixed bottom-0 justify-between bg-primary/40 backdrop-blur-sm'>
      {buttonData.map((button) => {
        return (
          <button
            key={button.id}
            name={button.name}
            className={`w-full ${pathname === button.name ? 'text-white' : 'text-gray-500'}`}
